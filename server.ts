...
app.put('/api/admin/orders/:id', authenticate, async (req, res) => {
  ...
  try {
    // First, get the old order data for logging
    const { data: oldOrder, error: fetchError } = await supabase.from('orders').select('*').eq('id', id).single();
    if (fetchError || !oldOrder) {
      return res.status(404).json({ error: 'Devis non trouvé' });
    }

    const updatedOrderData: OrderUpdate = { updated_at: new Date().toISOString() };
    if (status) updatedOrderData.status = status;
    if (finalPrice !== undefined) updatedOrderData.final_price = Number(finalPrice);

    const { data, error } = await supabase.from('orders').update(updatedOrderData).eq('id', id).select();
    ...
    // TODO: Implement Supabase-based notification system
    console.log('Notification (Order Status Update):', `Statut Commande ${updatedOrder.id} !`, `Le devis de M./Mme ${updatedOrder.client_name} a été basculé au statut : "${status}" par l'administrateur.`);

    // TODO: Implement Supabase-based audit logging
    console.log('Activity Log (Update Order):', user.id, user.email, user.role, `Mise à jour Devis/Statut : ${status}`, updatedOrder.id, 'Commande', oldOrder, updatedOrder);
...
app.delete('/api/admin/orders/:id', authenticate, async (req, res) => {
  const user = (req as any).user;
  ...
  try {
    // First, get the target order data for logging
    const { data: targetOrder, error: fetchError } = await supabase.from('orders').select('*').eq('id', id).single();
    ...
    // TODO: Implement Supabase-based audit logging
    console.log('Activity Log (Delete Order):', user.id, user.email, user.role, 'Suppression Commande Devis', id, 'Commande', targetOrder, undefined);
